...
class jobstatus {
    ...
    public function save($data) {
        ...
        $this->_db->where('job_status_name',$data['job_status_name']);
        $match = $this->_db->getOne('tms_job_status');
        ...
        if($match){
            ...
            $isDefault = $this->_db->getOne('tms_job_status');
            ...
            if($isDefault && $data['is_default'] == 1){
                ...
                $id = $this->_db->insert('tms_job_status', $data);
                if ($id) {
                    $return['status'] = 200;
    ...
    public function getAll() {
        $results = $this->_db->get('tms_job_status');
        return $results;
    }
    
    public function getTypeById($id) {
        $this->_db->where('jb_status_id', $id);
        $result = $this->_db->getOne('tms_job_status');
        return $result;
    }
    ...
    public function update($id, $data) {
        $this->_db->where('job_status_name',$data['job_status_name']);
        $match = $this->_db->getOne('tms_job_status');
        if($match && $match['jb_status_id'] !=$id){
                $return['status'] = 422;
                $return['msg'] = 'Status already exists.'; 
        }else{
            $this->_db->where('is_default',1);
            ...
            if($isDefault && $data['is_default'] == 1){
                ...
                $data['modified_date'] = date('Y-m-d H:i:s');
                $this->_db->where('jb_status_id', $id);
                $id = $this->_db->update('tms_job_status', $data);
                if ($id) {
                    $return['status'] = 200;
    ...
    public function delete($id) {
        ...
        if($ifIdUsedInGeneral){
            ...
            $this->_db->where('jb_status_id', $id);
            $defaultStatusNeverDel = $this->_db->getOne('tms_job_status');
            ...
            if($defaultStatusNeverDel['default_added'] == 1){
                ...
                $this->_db->where('jb_status_id', $id);
                $id = $this->_db->delete('tms_job_status');
                if ($id) {
                    $return['status'] = 200;
    ...
    public function proStatusget() {
        ...
        $result = $this->_db->get('tms_job_status');
        return $result;
    }
    public function proStatusgetOne() {
        ...
        if(!$result){
            $this->_db->where("is_active",1);
            $this->_db->where("job_status_name", '%progress%', 'like');
            $result = $this->_db->getOne('tms_job_status');
        }
        return $result;
    }
